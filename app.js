...
function loadLocal(){try{const s=JSON.parse(localStorage.getItem("sardegna-v17")||localStorage.getItem("sardegna-v16")||localStorage.getItem("sardegna-v10")||"{}");PLACES.forEach(p=>{if(s.favorites?.[p.id])p.favorite=true;if(s.visited?.[p.id])p.visited=true;if(s.notes?.[p.id])p.personalNote=s.notes[p.id]})}catch(e){}}
function saveLocal(){const favorites={},visited={},notes={};PLACES.forEach(p=>{if(p.favorite)favorites[p.id]=true;if(p.visited)visited[p.id]=true;if(p.personalNote)notes[p.id]=p.personalNote});localStorage.setItem("sardegna-v17",JSON.stringify({favorites,visited,notes}))}
...
function score(p){let s=p.wow*1.6+p.waterColor*.7+p.snorkeling*.7+seaScore(p)*1.2+textScore(p,state.query);if(state.mode==="wow")s+=p.wow*1.8+p.waterColor*1.4;if(state.mode==="snorkel")s+=p.snorkeling*2.4+(String(p.tags||"").includes("snorkeling")?8:0);if(state.mode==="local"){s+=isLocal(p)?80:-1000;}if(state.mode==="balanced")s+=p.wow+p.snorkeling*.8-(drive(p)/20)-(p.walkMin/8);if((p.windOK||"").includes(state.wind))s+=6;if((p.windAvoid||"").includes(state.wind))s-=12;s+=(drive(p)<=state.driveMax)?8:-(drive(p)-state.driveMax)/2;s+=(p.walkMin<=state.walkMax)?6:-8;if(state.quietOnly)s+=(p.crowdTarget==="Tranquillo"?10:-4);if(state.meteoAiOnly)s+=meteoAiScore(p)*1.8;if(state.localOnly&&!isLocal(p))s-=1000;if(state.withinDriveOnly&&drive(p)>state.driveMax)s-=1000;if(state.favoritesOnly&&!p.favorite)s-=1000;if(state.hideVisited&&p.visited)s-=1000;return Math.round(s*10)/10}
...
function why(p){const bits=[];if(state.mode==="snorkel"&&p.snorkeling>=7)bits.push("buono per snorkeling");if(state.mode==="wow"&&p.wow>=9)bits.push("mare wow");if((p.windOK||"").includes(state.wind))bits.push("vento favorevole");if(drive(p)<=state.driveMax)bits.push("entro tempo auto");if(p.walkMin<=state.walkMax)bits.push("camminata ok");if(p.name.toLowerCase().includes("liberotto"))bits.push("ottimi scogli laterali");if(state.meteoAiOnly&&meteoAiScore(p)>18)bits.push("meteo favorevole");return bits.slice(0,4).join(" · ")||"buon equilibrio generale"}
